...
const player_resource = preload("res://characters/player/player.tscn")
const world_resource = preload("res://world.tscn")

const ROOM_PATH = "res://data/rooms"
...
func room_load_scene(room_scene: PackedScene):
	if world.has_node("Room"):
		if room.has_node("Player"):
			room.remove_child(player)
		world.remove_child(world.get_node("Room"))
		room.queue_free()
